...
im-guard summary data/samples/sample_cases.jsonl
im-guard predict data/samples/sample_cases.jsonl --out outputs/demo_predictions.jsonl
im-guard eval outputs/demo_predictions.jsonl
```

没有模型权重时，CLI 使用启发式 Judge 演示协议闭环；有 fine-tuned checkpoint 时传 `--model-path outputs/im-audit-judge` 即可切到真实 Transformers 推理。
