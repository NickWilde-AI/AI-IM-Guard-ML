...
def summarize_results(requests: int, elapsed: float, status_counts: dict[int, int], latencies: list[float]) -> dict:
...
def run_benchmark(
    ...
    *,
    ...
    result = summarize_results(requests, elapsed, status_counts, latencies)
    result["url"] = url
...
def main() -> int:
    ...
    result = run_benchmark(args.url, args.requests, token=args.token, warmup=args.warmup, timeout=args.timeout)
