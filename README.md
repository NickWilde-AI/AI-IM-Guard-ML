...
[Quick Start](#quick-start) · [Architecture](#architecture) · [Training](#training) · [Enterprise Checks](#enterprise-checks) · [Docs](#documentation)
...
```mermaid
flowchart LR
  A["IM Review Request"] --> B["Evidence Builder"]
  B --> C["Prompt / Feature Rendering"]
  C --> D["LLM Judge"]
  D --> E["JSON Parsing"]
  E --> F["Postprocess Guardrails"]
  F --> G["Policy Routing"]
  G --> H["Audit Store"]
  G --> I["Metrics / Alerts"]
  I --> J["Hard Case Feedback"]
  J --> K["Dataset Builder"]
  K --> D
```
...
- unit tests and source compilation;
